...
import { TabbarRouterGroups } from '@/utils/router.js';
...
export const navigateTo = ({ url = '', type = 'navigateTo', delta = 1, checkIsConnected = true, events = {} }) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (TabbarRouterGroups.map((item) => item.path).includes(url.split('?')[0])) {
      type = 'relaunch';
    }
    switch (type) {
      case 'navigateTo':
